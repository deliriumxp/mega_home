...
class BundleStore:
    ...
    def __init__(self, hass: HomeAssistant, client: ManagerClient) -> None:
        ...
        # ⚠ Один прогон синхронизации за раз. Её зовут ДВА независимых
        # источника — опрос координатора (раз в 15 минут) и подсказка
        # `app_changed` из живого канала, — а менеджер на каждом переподключении
        # шлёт подсказки подряд, так что «одновременно» тут норма, а не
        # экзотика. Без замка получалось два зла сразу (разобрано 2026-09-08):
        #   • прогоны РАЗНЫХ версий: активной становилась та, что финишировала
        #     последней. Опрос, взявший манифест до публикации, но закончивший
        #     позже подсказки, возвращал дом на СТАРЫЙ интерфейс — и тот
        #     раздавался до следующего опроса, до 15 минут, молча;
        #   • прогоны ОДНОЙ версии: каталог-черновик один на версию
        #     (`.partial-<имя>`), и `_reset_dir` второго вычищал то, что писал
        #     первый, — активным мог стать неполный бандл, то есть белый экран.
        # Хвост той же гонки: `_prune` держит две последние версии по времени, и
        # старая активная могла быть удалена прямо из-под раздачи.
    ...
    async def async_sync(self, version: str | None = None) -> bool:
        ...
        ⚠ Прогон ровно один за раз (`self._lock`; чем это стоило — в `__init__`).
        Второй вызов не отбрасывается, а ЖДЁТ: подсказка, пришедшая во время
        скачивания, обязана быть отработана — иначе объект пропустил бы выпуск
        интерфейса до следующего опроса.

        ⚠ И манифест читается ВНУТРИ замка, а не до него. Прогон, успевший взять
        манифест до ожидания, дождался бы очереди и всё равно сделал активной
        свою — уже устаревшую — версию: замок снаружи чтения манифеста выглядит
        как починка, но ею не является.
        """
        async with self._lock:
            return await self._sync(version)

    async def _sync(self, version: str | None = None) -> bool:
        """Одна синхронизация. Зовётся ТОЛЬКО из `async_sync`, всегда под замком."""
        try:
