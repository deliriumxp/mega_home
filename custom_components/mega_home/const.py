...
# 2026-09-09 и переворачивает прежнюю запись «команда сразу после того, как
# поток открыт»: команда, отданная В ТУ ЖЕ МИЛЛИСЕКУНДУ, что RTSP PLAY, даёт
# РОВНО НОЛЬ байтов — и не «медленно», а навсегда, регистратор просто молчит.
# Та же команда через секунду после PLAY отдаёт первый байт мгновенно (0.0 с).
# Воспроизведено и на основном архиве, и на суб-. Отсчёт ведём от конца
# переговоров: go2rtc открывает RTSP ДО того, как отдаст SDP-ответ, поэтому
# «переговоры кончились» — это заведомо «PLAY уже был».
