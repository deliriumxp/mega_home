...
TRASSIR_LOGIN_GAP = 5
TRASSIR_TIMEOUT = 15
# ⚠ Срок ОТДЕЛЬНО для `/events`: это ДЛИННЫЙ ОПРОС, а не обычный запрос.
# Регистратор держит соединение, пока не случится событие, — замер стенда
# 2026-09-13 (`TRASSIR-4.8.2.0`, 12 камер с движением): 19.5, 43.1 и 46.7
# секунды подряд, и каждый раз в ответе РОВНО ОДНО событие. С общим сроком в
# 15 с дом обрывал опрос сам, и это стоило не только строки в журнале:
# оборванный запрос ТЕРЯЕТ события. Замер потери — рвущаяся сессия против
# эталонной, 60 секунд: эталон увидел 8 событий, рвущаяся получила 1, семь из
# восьми не дошли ВООБЩЕ. Отсюда правило: отвечать должен СЕРВЕР, а не наш
# таймер, поэтому срок заведомо больше его удержания.
TRASSIR_EVENTS_TIMEOUT = 120
