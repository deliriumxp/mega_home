    ...
    async def async_seek(
        ...
        if not want or want == old.quality:
            ...
            if old.started:
                ...
                # ⚠ ПОСЛЕ `seek` АРХИВ СТОИТ НА ПАУЗЕ — и это ГЛАВНАЯ причина
                # «любая перемотка, и видео не доходит». Замер объекта
                # 2026-09-13 (свой RTSP-читатель, канал IAtwTYwK, живой
                # регистратор): `play` даёт ровный поток ~200 КБ/с и курсор
                # 14:34:32 → 14:34:38, `state_desc: "P"`. Следом `seek` на
                # 17:54:44 отвечает `success: 1`, курсор ЧЕСТНО уезжает на
                # 17:54:44 — а данные прекращаются НАВСЕГДА, и `state_desc`
                # из "P" становится ПУСТЫМ. Тот же опыт с `play` следом:
                # данные идут сразу (899 → 2886 КБ), курсор 17:54:44 →
                # 17:55:09, "P" держится, и дыру между фрагментами архив
                # проходит сам.
                #
                # ⚠ Это НЕ противоречит «одна команда на соединение»: тот факт
                # про ВТОРОЙ `play` по уже играющему потоку. Здесь поток
