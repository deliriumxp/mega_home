...
class MegaHomeCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    def keep_polling(self, entry: MegaHomeConfigEntry) -> None:
        ...
        только пока у него есть слушатели, а слушатели — это сущности; у дома без
        штор (`cover.py`) их нет вовсе: интеграция — транспорт и раздатчик файлов. После первого опроса на старте дом больше не
