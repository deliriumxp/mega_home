...
# ⚠ Каталог БОЛЬШЕ НЕ НАПОЛНЯЕТСЯ (0.2.20): фоны едут общим каналом файлов.
# Имя осталось ровно затем, чтобы координатор один раз снёс старую копию с
# диска дома; ни хранилища, ни маршрута `api/stock-photo/` под него нет с
# 0.2.40 — они были вторым водопроводом за теми же байтами.
