...
## Сторож объекта: правила менеджера, которые дом крутит сам (0.2.39)

Проба (0.2.32) убрала из мониторинга WG-туннель, но инициатива осталась у менеджера: дом
шёл к устройству только по его просьбе. Значит объект, потерявший связь с менеджером, не
сторожил себя вовсе — а рвётся эта связь ровно вместе с тем, из-за чего объект и становится
интересен. `agent.py` переносит в дом ИНИЦИАТИВУ, не перенося сюда знание.

Правило — данные от менеджера: «периодически выполняй вот эти задания → проверь вот эти
условия → если нарушены дольше N → выполни вот эти задания, не чаще T и не больше K раз в
сутки → отчитайся». Задания — те же два примитива, что у пробы. Здесь НЕТ ни одной проверки:
первый сторож (перезагрузка зависшего директора Control4) собран целиком в менеджере, и
следующий не будет стоить релиза. Полный контракт и ловушки —
`mega-manager/docs/home-agent.md`.

Что здесь важно и почему:

- **Правила кэшируются на диске и читаются ДО первого визита к менеджеру.** Иначе объект без
  интернета после перезапуска Home Assistant переставал бы сторожить себя навсегда — то есть
  ровно то, ради чего сторож заводился, не работало бы.
- **Правила НЕ едут в теле конфига**, у них своя дверь с токеном объекта: тело конфига
  уходит браузеру жильца как есть, а в правиле стоят адрес устройства и команда действия. Тот
  же довод, что у учётки TRASSIR.
- **Отчёты вычёркиваются из очереди только после успеха** и ровно на ту длину, что
  отправляли: за время запроса сторож мог дописать событие.
- **Синхронизация зовётся из ОБЕИХ ветвей опроса координатора** — ветвь «версия конфига не
  изменилась» самый частый путь, и именно на ней когда-то не проверялся бандл.
- **Испорченное правило не роняет остальные** (`_valid`): сторож нужен целиком, а не до
  первой ошибки.

## Заготовки фонов: свой маршрут снят совсем (0.2.40)

Хвост уборки 0.2.20. Фоны комнат и плиток едут общим каналом файлов
(`api/asset/photo/room/*`, `photo/tile/*`), а рядом до сих пор лежали
`StockPhotoStore`, `stock_version` и маршрут `api/stock-photo/<key>` — второй
водопровод за теми же байтами. Хуже: `coordinator.stock_photos` перестал
существовать ещё в 0.2.20, поэтому маршрут отвечал AttributeError, а диагностика
интеграции падала ЦЕЛИКОМ — то есть инструмент, которым разбирают «почему в доме
не то», сам не работал. Снято всё; приложение этим маршрутом не пользуется с
2026-09-09. Из `const.py` осталось только имя каталога — по нему координатор
один раз сносит старую копию с диска дома.

## Кадр камеры, подправленный жильцом, — данные дома (0.2.67)

Решение заказчика 2026-09-15: фон плитки жилец правит сам с 0.1.15 (запись выше), а
кадр камеры — участок, который попадает в плитку, — до сих пор писал только менеджер
(`ClientTileCrops` у него, `tiles[].crop` в конфиге). Разница была искусственной, и
кадр получил ровно ту же архитектуру, что и фон: `crops.py` (`CropStore`, диск, имя
файла — хеш id плитки, как у `photos.py`) плюс пара дверей, `api/crops` (список) и
`api/crop/{tile}` (`POST`/`DELETE`), заведённых и локально (`http.py`), и в переносе
снаружи (`relay_api.py`) — та же дисциплина, что у фото: писать можно только КАМЕРУ
текущего состава (`crop_key_known`), диск не забить произвольным набором ключей.

⚠ Интеграция кадр НЕ ТОЛКУЕТ — только хранит `{x, y, w}` как прислали (после
`crop_value_valid`) и отдаёт списком по требованию. Слияние с заготовкой из конфига
(своя правка жильца поверх кадра, который подобрал инсталлятор) делает БАНДЛ
(`tile-crops.ts` в менеджере), не дом: та же граница «толкование живёт в приложении»,
которой уже почти десять лет следует `entity_view` (docs/plan-thin-integration.md).
Нового маршрута для СЛИЯНИЯ поэтому не заводится — только для ХРАНЕНИЯ, и это как раз
тот случай, когда общий канал не подходит ни один из трёх: `HomeConfig` знает только
то, что знает МЕНЕДЖЕР (а это правка жильца), `assets` — только файлы менеджера, а
`relay` требует дороги ДО менеджера, которой у объекта может не быть вовсе, — тот же
резон, что и у фото абзацем выше.

## Готовые варианты фото считает дом (0.2.69)

Решения заказчика 2026-09-17. Размытие фона комнаты и ч/б выключенной плитки делал БРАУЗЕР —
фильтрами CSS или холстом на каждом показе. На слабых устройствах объектов это была главная
цена кадра, а на iPad размытого фона не было вовсе: холст опирался на `ctx.filter`, которого
Safari без флага не знает. Теперь картинку в нужном виде готовит дом (`imaging.py`, Pillow — та
же библиотека, что в зависимостях ядра Home Assistant), а браузеру остаётся её нарисовать.

- ⚠ **Считает ТОЛЬКО дом.** «Иначе у нас будет два алгоритма и точно всплывут косяки»: вид одной
  картинки у двух реализаций разошёлся бы. Макет телефона у инсталлятора в менеджере берёт
  варианты у дома тем же переносом, что и кадры камер.
- ⚠ **Маршрутов нет — только query** к уже существующим `api/photo/{key}` и `api/asset/{key}`
  (`?w=1080&blur=14`, `?gray=1&dim=35`), в обеих дверях одним кодом (`photo_file`, `asset_file`).
  Без query отдаётся исходник: старый бандл не замечает ничего. Вид описан ОПЕРАЦИЯМИ, а не
  смыслом («размыть», а не «фон комнаты») — новый вид плитки не стоит релиза.
- ⚠ **Набор вариантов ограничен**: сторона приводится к ступеням, радиус и затенение — к
  пределам. HTTP дома без аутентификации, и свободные числа в адресе позволили бы забить диск.
- **Вариант под новое устройство считается при первом запросе и хранится**; имя несёт отметку
  исходника (время и размер файла), потому что фото жильца живёт под постоянным именем. При
  замене исходника уже запрошенные виды пересчитываются ЗАРАНЕЕ (`LookStore.refresh` после
  синхронизации файлов), чтобы устройства не ждали первого показа заново.
- ⚠ **Версия в адресе файла менеджера обязана совпасть с конфигом**, иначе 404. Ответ `immutable`,
  а макет у инсталлятора знает новую версию от менеджера раньше, чем дом её выкачал: без сверки
  под новым адресом навсегда закэшировалась бы старая картинка.
- Приложение узнаёт умение ОТВЕТОМ (`imaging: true` в `api/photos`), а не номером версии; без
  флага оно рисует вид фильтрами CSS, как раньше. `photo_looks` в диагностике — число вариантов
  на диске.

## Шлюз: закрытый список возможностей (0.3.0)

Решение заказчика 2026-09-19: шлюз умеет СРАЗУ всё, что может понадобиться любой вендорской
интеграции; релиз ради нового вендора — провал архитектуры. Список классов и почему он такой —
`docs/home-gateway.md` у менеджера (владелец), здесь только раскладка по файлам:
`access.py` (описание), `templating.py` (шаблоны и фильтры), `access_secrets.py` (учётка на
доступ), `access_http.py` (HTTP, виды авторизации, вход, поток, WebSocket), `access_raw.py`
(TCP/UDP), `access_ws.py` (вызов по WebSocket), `mqtt.py` + `mqtt_calls.py` (клиент 3.1.1),
`gateway.py` (политика, уровни), `ops_door.py` (операция), `media.py` (источник go2rtc по
шаблону), `device_events.py` и `listeners.py` (события устройств), `stream.py` (TCP/UDP-поток,
службы дома на loopback).

- ⚠ Дверь принадлежит ДОМУ (`__init__.py`), драйвер Trassir только регистрирует свою сессию
  (`bind_session`) для своего доступа. Выгрузка ЖДЁТ источники событий, потом закрывает дверь.
- ⚠ Вендорские запреты путей и пути долгого опроса — данные описания; прежние константы получает
  только описание прежней формы (без `auth`).
- ⚠ Список заперт `tests/test_gateway_classes.py::test_список_классов_заперт`; исправления ревью
  2026-09-19 — `tests/test_gateway_complete.py`.

## Шторы на импульсных реле (0.3.0)

Перенос интеграции `pulse_cover` (архив): логика — `core/pulse_blind.py` без HA, сущность —
`cover.py`. Шторы приходят конфигом объекта (`covers`), сущности появляются и снимаются на лету.
`entity_id` задаёт менеджер (у переехавших — прежний). Физика и правила хода —
`docs/ha-integration.md` у менеджера.

## SIP-мост домофонии: свой Asterisk (0.2.70, проверка)

Решение заказчика 2026-09-17: разговор с вызывной панелью — через SIP, готовым Asterisk рядом с
Home Assistant, а не аддоном (план и замер стенда — `docs/intercom-remote.md` у менеджера).
`sip_bridge.py` — та же схема, что свой go2rtc: мост принимает прямой IP-вызов панели и держит его
звонящим, пока телефон не наберёт `answer` (`sip_calls.py`).

- ⚠ **Включается КОНФИГОМ объекта** (`intercom.sipBridge`), а не релизом: релиз доезжает до всех
  домов, а мост ставит пакеты в их контейнер.
- ⚠ **Asterisk ставится `apk add` из репозитория Alpine образа HA** — go2rtc в образе есть,
  Asterisk нет. Контейнер пересоздаётся при обновлении HA, и установка повторяется: цена в
  `install_seconds` диагностики (замер в образе: 7–8 с). Без сети — отказ с причиной в `why`.
- ⚠ **Всё своё — в `.storage/mega_home_sip/`** через свой `asterisk.conf`; `/etc` контейнера не
  трогаем. Модули грузятся `autoload`: белый список зависимостей PJSIP легко недосчитать, а
  модули без конфига отказываются сами — их ERROR в логе при старте штатны.
- ⚠ **`apply()` не ждёт**: зовётся из цикла синхронизации конфига, а установка идёт секундами и
  минутами — работа уходит в фоновую задачу, повторный конфиг её не дублирует.
- **«Поднялся» — это `transport-udp` в `pjsip show transports`**, а не живой процесс (урок
  go2rtc). Сирота прошлого запуска отвечает на НАШ сокет управления — усыновляется.
- ⚠ **Вход — две двери, обе по адресу, учёток нет** (решение заказчика 2026-09-19): панели — с
  адресов `intercom.panels` конфига объекта (нет адресов — вызов не принимается ни от кого),
  телефон — с loopback. HTTP-сервер Asterisk (WebSocket телефона и ARI) слушает только
  `127.0.0.1`, а `stream.py` пускает loopback ровно на его порт: телефон доходит до моста лишь
  каналом менеджера, права жильца проверяет менеджер, из Wi-Fi объекта мост не виден. По имени из
  `From` не опознаём (`endpoint_identifier_order=ip`) — чужой вызов с именем `resident` стал бы
  телефоном.
