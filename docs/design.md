...
## Шлюз: закрытый список возможностей (0.2.74)
...
`access.py` (описание), `access_secrets.py` (учётка на доступ), `access_http.py` (HTTP и виды
авторизации), `access_raw.py` (TCP/UDP), `mqtt.py` (клиент 3.1.1), `gateway.py` (политика,
уровни), `ops_door.py` (операция), `media.py` (источник go2rtc по шаблону), `device_events.py` и
`listeners.py` (события устройств), `stream.py` (TCP/UDP-поток, службы дома на loopback).

- ⚠ Вендорские запреты путей и пути долгого опроса — данные описания; прежние константы живут
  только как совместимость для описаний без этих ключей (`access.LEGACY_*`).
- ⚠ Живая сессия и учётка драйвера Trassir привязаны к ЕГО доступу (`provider_access`): второй
  доступ с входом по сессии их не получает.
- ⚠ Список заперт `tests/test_gateway_classes.py::test_список_классов_заперт`.
