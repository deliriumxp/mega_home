...
`sip_bridge.py` — та же схема, что свой go2rtc; сейчас мост только принимает прямой IP-вызов из
локальной сети, отвечает через 5 с и возвращает звук и видео эхом.
...
- Вызов без регистрации — только из частных сетей (`identify`): мост без аутентификации.
