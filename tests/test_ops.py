...
def test_команда_превращается_в_вызов_службы():
    ...
    # ⚠ Только «выполнено»: новое состояние приносит поток (`state_changed`),
    # а снимок сразу после вызова выдавал старое за результат (у KNX — до
    # телеграммы подтверждения). Контракт HA: dev-api-websocket.md менеджера.
