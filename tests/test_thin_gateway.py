...
def test_операции_канала_и_маршруты_api_заперты() -> None:
    ...
    locked_routes = {
        ...
        "api/intercom",
        # вложения к событиям (кадр гостя) — хранилище жильца на объекте, часть
        # F (`docs/plan-event-attachments.md` менеджера)
        "api/event-file",
        # лог разработки от приложения внутри дома — канал, часть A: сессии
        # менеджера на странице дома нет (`docs/plan-dev-logs.md`, шаг 4)
        "api/dev-log",
    }
...
def test_размер_ядра_не_растёт_прикладной_логикой() -> None:
    """Замок 5 плана: КОД `core/*.py` ≤ 5 000 строк суммарно.
    ...
    ⚠ 4 800 → 5 000 (2026-09-20), и причина ровно одна: ось «время жизни» —
    удержание соединения для приложения жильца (виды `ws` и `http` у сессии,
    форма `ConnectRequest` вместо `host`/`port`, потолок на весь дом). Это
    транспорт, часть C, и план сам обещал «около 5 000». Замер после правки —
    4 982; дубль проверок адреса при этом СНЯТ, их ведёт
    `connect.resolve_address`, как у одиночного вызова. Следующий подъём обязан
    назвать свою ось так же: «выросло прикладное» подъёмом не лечится.

    ⚠ 5 000 → 5 200 (2026-09-22, пачка 0.5.6), оси две и обе — механизмы
    утверждённых планов, а не прикладная логика: лог разработки (канал, часть A:
    обработчик `logging`, очередь, кадр `dev-log`) и вложения к событиям
    (хранилище жильца, часть F: правило «на событие — GET», файлы с пределами).
    Замер — 5 119; в той же пачке ядро СОКРАТИЛОСЬ на ~120 строк сведением
    трёх HTTP-клиентов и двух маршрутизаторов в один.
    """
